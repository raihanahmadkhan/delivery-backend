...
def parse_csv(content: bytes) -> List[dict]:
    ...
    for row_num, row in enumerate(reader, start=2):
        ...
        try:
            ...
            raise ValueError(f'Row {row_num}: invalid coordinate values - {exc}') from exc
        if not -90 <= lat <= 90:
        ...
        point = {'id': int(norm_row.get('id', row_num - 2)), 'name': norm_row.get('name', f'Node {row_num - 2}'), 'latitude': lat, 'longitude': lon, 'demand': float(norm_row.get('demand', 0) or 0), 'time_window_start': float(norm_row.get('time_window_start', 0) or 0), 'time_window_end': float(norm_row.get('time_window_end', 1440) or 1440)}
        points.append(point)
    if not points:
