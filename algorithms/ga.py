...
class GeneticAlgorithm(BaseOptimizer):
    ...
    def optimize(self) -> OptimizationResult:
        ...
        population: List[List[int]] = [self._random_individual() for _ in range(self.population_size)]
        ...
        for generation in range(self.n_generations):
            ...
            if gen_best_cost < best_cost:
                ...
                _, best_violations, best_details = self.constraint_penalty(self._to_full_route(best_individual))
            history.append(best_cost)
            sorted_pop = [ind for _, ind in sorted(zip(fitness_scores, population), key=lambda x: x[0])]
            next_gen: List[List[int]] = [ind[:] for ind in sorted_pop[:self.elitism_count]]
        ...
        return OptimizationResult(route=full_route, cost=self.route_cost(full_route), history=history, time_ms=self._elapsed_ms(start), algorithm='GA', violations=best_violations, violation_details=best_details)
    ...
    def _tournament_select(self, population: List[List[int]], fitness: List[float]) -> List[int]:
        k = min(self.tournament_size, len(population))
        candidates = random.sample(range(len(population)), k)
        best_idx = min(candidates, key=lambda i: fitness[i])
        return population[best_idx][:]

    def _ox_crossover(self, parent_a: List[int], parent_b: List[int]) -> Tuple[List[int], List[int]]:
        size = len(parent_a)
