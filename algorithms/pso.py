...
class ParticleSwarmOptimizer(BaseOptimizer):
    ...
    def optimize(self) -> OptimizationResult:
        ...
        positions: List[List[int]] = [
            self._random_position() for _ in range(self.n_particles)
        ]
        ...
        pbest_costs:     List[float]      = [
            self._fitness(pos) for pos in positions
        ]
        ...
        for iteration in range(self.n_iterations):
            for i in range(self.n_particles):
                ...
                if current_cost < gbest_cost:
                    ...
                    _, best_violations = self.constraint_penalty(
                        self._to_full_route(gbest_position)
                    )
            ...
            for i in range(self.n_particles):
                ...
                cognitive_swaps = self._position_difference(
                    positions[i], pbest_positions[i]
                )
                
                social_swaps = self._position_difference(
                    positions[i], gbest_position
                )

                
                new_velocity: Velocity = self._apply_inertia(
                    velocities[i], self.inertia_weight
                )

                
                new_velocity += self._filter_swaps(
                    cognitive_swaps, self.c1 * r1
                )

                
                new_velocity += self._filter_swaps(
                    social_swaps, self.c2 * r2
                )
    ...
    def _position_difference(
        self, source: List[int], target: List[int]
    ) -> Velocity:
        ...
        for target_idx, target_val in enumerate(target):
            ...
            working[target_idx], working[source_idx] = (
                working[source_idx], working[target_idx],
            )

        return swaps

    def _apply_velocity(
        self, position: List[int], velocity: Velocity
    ) -> List[int]:
    ...
    def _apply_inertia(
        self, velocity: Velocity, weight: float
    ) -> Velocity:
