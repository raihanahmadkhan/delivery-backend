...
def run_benchmark(
    ...
    for algo in algorithms:
        ...
        if best_response is not None:
            results.append(
                BenchmarkEntry(
                    algorithm=best_response.algorithm,
                    route=best_response.route,
                    route_names=best_response.route_names,
                    cost=best_response.cost,
                    history=best_response.history,
                    time_ms=best_response.time_ms,
                    violations=best_response.violations,
    ...
    random_result = next(
        (r for r in results if r.algorithm.lower() == "random"), None
    )
    if random_result and random_result.cost > 0:
        for entry in results:
            if entry.algorithm.lower() != "random":
                improvement = (
                    (random_result.cost - entry.cost) / random_result.cost
                ) * 100
...
def _build_summary(results: List[BenchmarkEntry]) -> dict:
    ...
    return {
        ...
        "worst_cost":      round(worst.cost, 4),
        "cost_range":      round(worst.cost - best.cost, 4),
