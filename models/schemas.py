...
class OptimizeRequest(BaseModel):
    points: List[DeliveryPoint] = Field(..., min_length=2, description='List of delivery points. First point is treated as depot.')
    algorithm: Literal['aco', 'ga', 'pso', 'nearest_neighbor', 'random'] = Field(default='aco', description='Which algorithm to run.')
    params: dict = Field(default_factory=dict, description='Algorithm hyper-parameters (merged with defaults).')
    constraints: Constraints = Field(default_factory=Constraints, description='VRP constraints (capacity, time windows, vehicles).')
    distance_mode: Literal['haversine', 'euclidean'] = Field(default='haversine', description='haversine = real GPS distances; euclidean = abstract x,y.')
...
class OptimizeResponse(BaseModel):
    ...
    violation_details: List[str] = Field(default_factory=list, description='Array of violation-specific strings.')
    vehicle_routes: List[List[int]] = Field(default_factory=list)

class BenchmarkRequest(BaseModel):
    points: List[DeliveryPoint] = Field(..., min_length=2)
    algorithms: List[Literal['aco', 'ga', 'pso', 'nearest_neighbor', 'random']] = Field(default=['aco', 'ga', 'pso', 'nearest_neighbor', 'random'], description='Algorithms to include in the benchmark.')
    params: dict = Field(default_factory=dict, description='Shared params applied to all algorithms (algorithm-specific keys respected).')
    constraints: Constraints = Field(default_factory=Constraints)
    distance_mode: Literal['haversine', 'euclidean'] = Field(default='haversine')
    n_runs: int = Field(default=1, ge=1, le=10, description='Number of independent runs per algorithm; best result is returned.')
...
class BenchmarkEntry(BaseModel):
    ...
    improvement_vs_random: Optional[float] = Field(None, description='% distance reduction compared to Random baseline.')

class BenchmarkResponse(BaseModel):
    results: List[BenchmarkEntry]
    summary: dict = Field(default_factory=dict, description='Aggregate stats: best algorithm, worst algorithm, range, etc.')
